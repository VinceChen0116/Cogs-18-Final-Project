...
def decryption(message,code):  # My decryption function.
    ...
    for item in message:    # loop thru all elements in input message
        if ord(item)>=65 and ord(item)<=90:   # check if the input element is A-Z, and if so, we will decrypt it.
            decrypt_upper=chr(ord(item)-code)  # We calculate the shifted result by switching it to ASCII then back.
            decrypt_result=decrypt_result+decrypt_upper   # We add the bit to our created string.
        elif ord(item)>=97 and ord(item)<=122:  # check if the input element is a-z, and if so, we will decrypt it.
            decrypt_lower=chr(ord(item)-code)  # We again calculate by switching it to ASCII and then switch back.
            decrypt_result=decrypt_result+decrypt_lower   # We add the bit to our created string.
        else:
...
def encryption(message,code):  # My encryption function.
    ...
    code=code%26    # To make sure we always get correct shift bits if input shift bit is greater than 26                         
    for item in message:   # loop thru all elements in input message
        if ord(item)>=65 and ord(item)<=90:   # check if the input element is A-Z, and if so, we will encrypt it.
            encrypt_upper=chr(ord(item)+code)   # We calculate the shifted result by switching it to ASCII then back.
            ...
            encrypt_lower=chr(ord(item)+code)   # We again calculate by switching it to ASCII and then switch back.
...
def mainfunction(code,message,EDtype):  # Create our main function to output the correct result 
    if EDtype == 1:   # As the notebook file stated, if EDtype is 1, then it means that we will do ecnryption.
        return(encryption(message,code))  # Returned our encrypted result correctly.
    elif EDtype == 2:  # As the notebook file stated, if EDtype is 2, then it means that we will do decryption.
        return(decryption(message,code))  # Returned our decrypted result correctly.
    else:
